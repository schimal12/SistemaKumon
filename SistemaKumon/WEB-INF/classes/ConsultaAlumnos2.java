...
public class ConsultaAlumnos2 extends HttpServlet{
	...
	try{
	    ...
	    ResultSet rs = stat.executeQuery("SELECT * FROM alumno AS a WHERE a.EstatusPago = TRUE AND NOT EXISTS(SELECT al.ID FROM alumno AS al, sesion WHERE a.ID = ID_Alumno AND al.ID = ID_Alumno AND DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= FechaSesion);");
	    while(rs.next()){
