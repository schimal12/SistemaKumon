...
public class ConsultaPagos extends HttpServlet{
	...
	try{
	    ...
	    ResultSet rs = stat.executeQuery("SELECT TipoPago, SUM(Monto) FROM pago WHERE DATE_SUB(CURDATE(),INTERVAL 30 DAY) <= FechaPago GROUP BY TipoPago;");

	    while(rs.next()){
			datos.add(rs.getString("TipoPago"));
			datos.add(rs.getInt("SUM(Monto)"));
	    }
	    ...
	    RequestDispatcher disp = getServletContext().getRequestDispatcher("/pagosConsulta.jsp");
