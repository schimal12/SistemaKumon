...
public class ConsultaInasitencias1 extends HttpServlet{
	...
	try{
	    ...
	    ResultSet rs = stat.executeQuery("SELECT a.ID, a.Nombre, COUNT(a.ID) FROM alumno AS a, sesion WHERE a.ID = ID_Alumno AND DATE_SUB(CURDATE(),INTERVAL 7 DAY) <= FechaSesion GROUP BY a.ID;");
	    ...
	    while(rs1.next()){
			...
			if(rs1.getInt("ID") == idAux){
				...
				int auxiliar = 7 - rs.getInt("Count(a.ID)");
				datos.add(auxiliar);
			}else{
				rs.previous();
